...
const getLecturesDone = async (studentId: string, chapterId: number, subject: Subject): Promise<LecturesDoneResponse['data']> => {
...
export default function TargetAssignment() {
  ...
  const renderOngoingChaptersBox = (targetType: TargetType) => (
    <Card className="mt-4 bg-white shadow-md">
      <CardHeader>
        <CardTitle className="text-pcb text-xl sm:text-2xl">Ongoing Chapters</CardTitle>
      </CardHeader>
      <CardContent>
        <Tabs defaultValue="physics" className="w-full">
          <TabsList className="bg-pcb/5 flex justify-evenly mb-4">
            {['physics', 'chemistry', 'biology'].map(subject => (
              <TabsTrigger key={subject} value={subject} className="data-[state=active]:bg-pcb data-[state=active]:text-white w-full text-xs sm:text-sm py-1 sm:py-2">
          ...
          {['physics', 'chemistry', 'biology'].map(subject => (
            <TabsContent key={subject} value={subject}>
              {ongoingChapters[targetType][subject as Subject].map(chapter => (
                <div key={chapter.chapterId} className="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-4 pb-4 border-b last:border-b-0">
                  <span className="font-medium mb-2 sm:mb-0">{syllabus[subject as Subject].find(c => c.id === chapter.chapterId)?.chapterName}</span>
                  <div className="grid grid-cols-2 sm:flex sm:items-center gap-2 sm:space-x-4">
                    <div className="flex items-center space-x-2">
                      <Label htmlFor={`lectures-per-day-${targetType}-${subject}-${chapter.chapterId}`} className="text-xs sm:text-sm whitespace-nowrap">
                        Lectures/day:
                      </Label>
                      <Input id={`lectures-per-day-${targetType}-${subject}-${chapter.chapterId}`} type="number" value={chapter.lecturesPerDay} onChange={e => handleLecturesPerDayChange(targetType, subject as Subject, chapter.chapterId, parseInt(e.target.value))} className="w-12 h-8 text-sm" min="1" />
                    </div>
                    <div className="flex items-center space-x-2">
                      <Label htmlFor={`lectures-done-${targetType}-${subject}-${chapter.chapterId}`} className="text-xs sm:text-sm whitespace-nowrap">
                        Lectures done:
                      </Label>
                      <span id={`lectures-done-${targetType}-${subject}-${chapter.chapterId}`} className="text-sm font-medium">
                        {chapter.lecturesDone}
                      </span>
                    </div>
                    <div className="flex items-center space-x-2 col-span-2 sm:col-span-1">
                      <Label htmlFor={`complete-${targetType}-${subject}-${chapter.chapterId}`} className="text-xs sm:text-sm">
                        Complete:
                      </Label>
                      <Switch id={`complete-${targetType}-${subject}-${chapter.chapterId}`} checked={chapter.isComplete} onCheckedChange={checked => handleMarkComplete(targetType, subject as Subject, chapter.chapterId, checked)} />
                    </div>
                  </div>
                </div>
              ))}
  ...
  const renderSubjectTable = (targetType: TargetType, subject: Subject) => (
    <Card className="mt-4 bg-white shadow-md">
      <CardHeader>
        <CardTitle className="text-pcb">{subject.charAt(0).toUpperCase() + subject.slice(1)}</CardTitle>
      </CardHeader>
      <CardContent>
              ...
              <tr className="border-b border-pcb/20">
                ...
                {[0, 1, 2].map((_, index) => (
                  <th key={index} className="p-2">
                    <Select onValueChange={value => handleChapterSelect(targetType, subject, index, Number(value))} value={selectedChapters[targetType][subject][index].toString()}>
            ...
            <tbody>
              {dates.map(date => (
                <tr key={date} className="border-b border-pcb/10">
                  <td className="p-2 text-pcb text-center sticky left-0 bg-white z-10">{format(new Date(date), 'MMM dd')}</td>
                  {[0, 1, 2].map((_, index) => (
                    <td key={`${date}-${index}`} className="p-2 text-center">
                      <Checkbox id={`target-${targetType}-${subject}-${date}-${index}`} className="border-pcb/30 text-pcb" checked={checkboxStates[`${date}-${targetType}-${subject}-${index}`] || false} onCheckedChange={checked => handleTargetChange(date, targetType, subject, index, checked as boolean)} disabled={selectedChapters[targetType][subject][index] === 0} />
                    </td>
                  ))}
                </tr>
              ))}
      ...
      <CardHeader>
        <div className="flex items-center space-x-2">
          <Checkbox
            ...
            checked={targetType === 'Regular' ? showRegular : targetType === 'Revision' ? showRevision : showExtra}
          ...
          />
          <Label htmlFor={`${targetType}-targets-checkbox`} className="text-pcb">
            {targetType} Targets
          </Label>
        </div>
      </CardHeader>
      {((targetType === 'Regular' && showRegular) || (targetType === 'Revision' && showRevision) || (targetType === 'Extra' && showExtra)) && (
        <CardContent>
          ...
          <Tabs defaultValue="physics" className="w-full mt-4">
            <TabsList className="bg-pcb/5 flex justify-evenly">
              <TabsTrigger value="physics" className="data-[state=active]:bg-pcb data-[state=active]:text-white w-full">
                Physics
              </TabsTrigger>
              <TabsTrigger value="chemistry" className="data-[state=active]:bg-pcb data-[state=active]:text-white w-full">
                Chemistry
              </TabsTrigger>
              <TabsTrigger value="biology" className="data-[state=active]:bg-pcb data-[state=active]:text-white w-full">
                Biology
              </TabsTrigger>
            </TabsList>
            <TabsContent value="physics">{renderSubjectTable(targetType, 'physics')}</TabsContent>
            <TabsContent value="chemistry">{renderSubjectTable(targetType, 'chemistry')}</TabsContent>
            <TabsContent value="biology">{renderSubjectTable(targetType, 'biology')}</TabsContent>
          </Tabs>
          <Button onClick={addDay} className="mt-4 bg-pcb text-white hover:bg-pcb/90">
